...
func (c *Controller) enqueueSyncTargetKey(syncTargetKey string) {
	...
	for gvr := range informers {
		...
		stateLabelObjs, err := inf.Informer().GetIndexer().ByIndex(tmcindexers.ByClusterResourceStateLabelKey, workloadv1alpha1.ClusterResourceStateLabelPrefix+syncTargetKey)
		if err != nil {
			runtime.HandleError(err)
			continue
		}
		syncerFinalizerObjs, err := inf.Informer().GetIndexer().ByIndex(tmcindexers.BySyncerFinalizerKey, shared.SyncerFinalizerNamePrefix+syncTargetKey)
		if err != nil {
			runtime.HandleError(err)
			continue
		}
