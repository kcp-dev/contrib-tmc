...
func (o *BindComputeOptions) Run(ctx context.Context) error {
	...
	userWorkspaceTmcClient, err := tmcclient.NewForConfig(config)
	if err != nil {
		return fmt.Errorf("failed to create tmc client: %w", err)
	}

	// apply APIBindings
	bindings, err := o.applyAPIBinding(ctx, userWorkspaceKcpClient, userWorkspaceTmcClient, sets.New[string](o.APIExports...))
	if err != nil {
		return err
	}
	...
	// apply placement
	if err := o.applyPlacement(ctx, userWorkspaceTmcClient); err != nil {
		return err
	}

	// and wait for it to be ready
	if err := wait.PollImmediate(time.Millisecond*500, o.BindWaitTimeout, func() (done bool, err error) {
		placement, err := userWorkspaceTmcClient.SchedulingV1alpha1().Placements().Get(ctx, o.PlacementName, metav1.GetOptions{})
		if err != nil {
			return false, err
		}
...
func (o *BindComputeOptions) applyAPIBinding(ctx context.Context, kcpClient kcpclient.Interface, tmcClient tmcclient.Interface, desiredAPIExports sets.Set[string]) ([]*apisv1alpha1.APIBinding, error) {
	apiBindings, err := kcpClient.ApisV1alpha1().APIBindings().List(ctx, metav1.ListOptions{})
	if err != nil {
		return nil, err
	}
	...
	for i := range apiBindings.Items {
		...
		if localPath.Empty() {
			cluster, err := kcpClient.CoreV1alpha1().LogicalClusters().Get(ctx, corev1alpha1.LogicalClusterName, metav1.GetOptions{})
			if err != nil {
				return nil, err
			}
	...
	for export := range diff {
		...
		binding, err := kcpClient.ApisV1alpha1().APIBindings().Create(ctx, apiBinding, metav1.CreateOptions{})
...
func (o *BindComputeOptions) applyPlacement(ctx context.Context, client tmcclient.Interface) error {
