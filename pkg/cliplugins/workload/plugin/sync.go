...
func (o *SyncOptions) applySyncTarget(ctx context.Context, kcpClient kcpclient.Interface, tmcClient tmcclient.Interface, syncTargetName string, labels map[string]string) (*workloadv1alpha1.SyncTarget, error) {
	...
	syncTarget, err := tmcClient.WorkloadV1alpha1().SyncTargets().Get(ctx, syncTargetName, metav1.GetOptions{})
	...
	case apierrors.IsNotFound(err):
		...
		syncTarget, err = tmcClient.WorkloadV1alpha1().SyncTargets().Create(ctx,
	...
	if syncTarget, err = tmcClient.WorkloadV1alpha1().SyncTargets().Patch(ctx, syncTargetName, types.MergePatchType, patchBytes, metav1.PatchOptions{}); err != nil {
...
func (o *SyncOptions) getResourcesForPermission(ctx context.Context, config *rest.Config, syncTargetName string) (sets.Set[string], error) {
	tmcClient, err := tmcclient.NewForConfig(config)
	if err != nil {
...
func (o *SyncOptions) enableSyncerForWorkspace(ctx context.Context, config *rest.Config, syncTargetName, namespace string, labels map[string]string) (saToken string, syncerID string, syncTarget *workloadv1alpha1.SyncTarget, err error) {
	...
	tmcClient, err := tmcclient.NewForConfig(config)
	if err != nil {
		return "", "", nil, fmt.Errorf("failed to create tmc workload client: %w", err)
	}

	syncTarget, err = o.applySyncTarget(ctx, kcpClient, tmcClient, syncTargetName, labels)
	if err != nil {
