...
func TestSyncerTunnel(t *testing.T) {
	...
	upstreamTmcClient, err := tmcclientset.NewForConfig(syncerFixture.SyncerConfig.UpstreamConfig)
	require.NoError(t, err)

	syncTarget, err := upstreamTmcClient.Cluster(synctargetWsPath).WorkloadV1alpha1().SyncTargets().Get(ctx,
...
func TestSyncerTunnelFilter(t *testing.T) {
	...
	tmcClient, err := tmcclientset.NewForConfig(kcpServer.BaseConfig(t))
	require.NoError(t, err)

	kcpClient, err := kcpclientset.NewForConfig(kcpServer.BaseConfig(t))
	require.NoError(t, err)

	syncTarget, err := tmcClient.Cluster(syncerFixture.SyncerConfig.SyncTargetPath).WorkloadV1alpha1().SyncTargets().Get(ctx,
		syncerFixture.SyncerConfig.SyncTargetName,
		metav1.GetOptions{},
	)
	require.NoError(t, err)
	...
	require.NoError(t, err)
	require.NotEmpty(t, locatorJSON)
